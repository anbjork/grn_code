...
def compile_results_simulated(output_path):
    anton_util.log_timestamp('compiling results...')
    anton_util.log_timestamp('reading data...')

    benchmarks = anton_util.unpickle_object(f'{output_path}/simulated/benchmarks.pkl')
    ...
    output_path = f'{output_path}/simulated/compiled_results'
    ...
    if CONFIG_write_auroc_and_aupr:
        ...
        plot_out_dir = f'{output_path}/simulated/plots/roc_and_pr/'
        ...
        for ii, (ps, rs) in enumerate(zip(parameters_flat, benchmark_results)):
            for plot_type in ['plot_roc', 'plot_pr']:
                ...
                filename = f'{output_path}/simulated/plots/roc_and_pr/{tag}.svg'
